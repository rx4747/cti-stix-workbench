...
import {
  objectSchemaValidators,
} from "./generated/schema-validator-map.mjs";
...
export function validateCompiledBundle(
  value: unknown,
): SchemaValidationResult {
  if (!isRecord(value)) {
  ...
  const envelopeResult = runSchemaValidator(
    validateBundleSchema,
    envelope,
  );
  ...
  for (const [index, object] of value.objects.entries()) {
    const type = isRecord(object) && typeof object.type === "string"
      ? object.type
      : undefined;
    const validator = type === undefined
      ? undefined
      : objectSchemaValidators[type];
    ...
    errors.push(
      ...result.errors.map((error) => prefixObjectError(error, index)),
    );
  }
...
export function validateCompiledIndicator(
  value: unknown,
): SchemaValidationResult {
