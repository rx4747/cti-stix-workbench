...
function resolveReference(
  ...
  const target = wikiMatch?.[1] === undefined
    ? undefined
    : normalizeWikiTarget(wikiMatch[1]);
  const resolvedLink = target === undefined
    ? undefined
    : draft.links.find(
      (link) =>
        link.targetPath !== undefined
        && normalizeWikiTarget(link.raw) === target,
    );
  const assigned = resolvedLink?.targetPath === undefined
    ? undefined
...
function identityReferenceValue(
  ...
  const resolveOne = (candidate: unknown): {
    ...
    const link = draft.links.find(
      (item) =>
        item.targetPath !== undefined
        && normalizeWikiTarget(item.raw) === target,
    );
    const id = link?.targetPath === undefined
      ? undefined
...
function propertiesForScoIdentity(
  ...
  for (const fieldName of definition.idContributingProperties ?? []) {
    ...
    const mapped = fieldName === "extensions"
...
async function assignDrafts(
  ...
  for (const draft of drafts) {
    ...
    if (
      ...
      || !AUTHORABLE_FAMILIES.has(definition.family)
    ) {
      diagnostics.push(
        mappingDiagnostic(
    ...
    const existingId = draft.stixId
      ?? (typeof draft.properties.id === "string"
        ? draft.properties.id
        : undefined);
    candidates.push({
      draft,
  ...
  for (const candidate of candidates) {
    if (candidate.existingId !== undefined) {
      ...
      assign(
        candidate,
        identifierService.ensureUuid4(candidate.definition.type),
      );
    }
  }
  ...
  let pending = candidates.filter(
    (candidate) =>
      candidate.existingId === undefined
      && candidate.definition.family === "sco",
  );
...
function mapExtensions(
  ...
  for (const [extensionType, payload] of Object.entries(value)) {
    ...
    const fieldByName = new Map(
      extension.fields.map((field) => [field.name, field]),
    );
    ...
    for (const [key, nested] of Object.entries(payload)) {
      ...
      const mapped = extensionField === undefined
        ? jsonValue(
          nested,
          assigned.draft,
          "extensions",
...
export async function mapGraphToBundle(
  ...
  const assignedByPath = new Map(
    assigned.map((item) => [item.draft.path, item]),
  );
  ...
  for (const declaration of relationshipDeclarations) {
    ...
    if (source === undefined) {
      ...
      let identity =
        input.relationshipIdentities?.[key]
        ?? generatedRelationships.get(key);
      if (identity === undefined) {
        ...
        const validated = identifierService.ensureUuid4(
          "relationship",
          identity.id,
        );
        if (!validated.ok) {
  ...
  const bundleIdentifier = input.bundleId === undefined
  ...
  const objects = [...objectsById.values()].sort(
