...
function noteDeclaresStix(input: UntrustedNoteInput): boolean {
  ...
  return typeof input.frontmatter.stix_type === "string"
...
async function collectGraph(
  ...
  while (queue.length > 0) {
    ...
    for (
      const path of targetPaths(
        parsed.draft,
        settings.includeContextualLinks,
      )
    ) {
      if (!visited.has(path)) {
...
export async function exportActiveGraph(
  ...
  const validated = await validateActiveGraph(
    host,
    rootPath,
    settings,
    dependencies,
