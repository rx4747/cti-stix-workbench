...
const validationRoot = path.join(packageRoot, "src/validation");
const generatedRoot = path.join(validationRoot, "generated");
const schemaOutput = path.join(generatedRoot, "schema-validators.mjs");
const schemaDeclarationOutput = path.join(
  generatedRoot,
  "schema-validators.d.mts",
);
const schemaMapOutput = path.join(
  generatedRoot,
  "schema-validator-map.mjs",
);
...
const objectSchemaEntries = schemaEntries
  .filter((entry) => {
    ...
    return (
      ...
      || (
        family === "common"
        && commonObjectSchemas.has(path.basename(relativePath))
      )
    );
  })
  .map((entry) => ({
...
const validatorModule =
  ...
  + generatedValidatorModule.replaceAll(
    ucs2RuntimeExpression,
    "stixUcs2Length",
  );
