...
for (const entry of entries) {
  ...
  assert.equal(stats.isSymbolicLink(), false, `${entry.name} must not be a symlink`);
}

assert.deepEqual(
  JSON.parse(await readFile(path.join(releaseRoot, "manifest.json"), "utf8")),
  manifest,
);
...
for (const filename of filenames) {
  ...
  console.log(`${createHash("sha256").update(bytes).digest("hex")}  ${filename}`);
}
