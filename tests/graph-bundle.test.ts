...
import { mapGraphToBundle, relationshipIdentityKey } from "../src/core/graph-mapper";
...
describe("complete STIX graph mapping", () => {
  ...
  it("resolves nested refs and explicit relationships into a stable Bundle", async () => {
    ...
    const key = relationshipIdentityKey(indicatorId, "based-on", addressId);
    const relationshipIdentities: Readonly<
  ...
  it("returns generated note and relationship identities for persistence", async () => {
    const indicator = {
      ...
      links: [{ raw: "Address", targetPath: "Objects/Address.md" }],
      relationships: [
        {
          sourceNotePath: "Objects/Indicator.md",
          relationshipType: "based-on",
    ...
    const randomValues = [uuid4(11), uuid4(12), uuid4(13)];
    let randomIndex = 0;
  ...
  it("resolves ID-contributing SCO refs before creating UUIDv5", async () => {
    ...
    const message = {
      ...
      links: [{ raw: "Sender", targetPath: "Objects/Sender.md" }],
    } satisfies NormalizedStixDraft;
