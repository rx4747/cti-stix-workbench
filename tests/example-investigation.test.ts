...
function parseFixtureFrontmatter(markdown: string): Record<string, unknown> {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    const match = /^([a-z][a-z0-9_]*):(?:\s*(.*))?$/u.exec(lines[index] ?? "");
    ...
    while (/^\s+-\s+/u.test(lines[index + 1] ?? "")) {
      index += 1;
      items.push(parseScalar((lines[index] ?? "").replace(/^\s+-\s+/u, "")));
    }
    if (items.length > 0) {
...
class ExampleVaultHost implements ActiveGraphHost {
  ...
  async load(): Promise<void> {
    ...
    const pathsByBasename = new Map(
      exampleNotePaths.map((notePath) => [path.basename(notePath, ".md"), notePath]),
    );
    for (const [notePath, markdown] of contents) {
      const links = [...markdown.matchAll(/\[\[([^\]]+)\]\]/gu)].flatMap((match) => {
...
describe("fictional alpha investigation", () => {
  it("exports the committed example through the active-graph workflow", async () => {
    ...
    await host.load();
    const rootNote = host.notes.get(rootNotePath);
    if (rootNote === undefined) {
      throw new Error("Expected example investigation root note.");
    }
    const rootLinks = new Set(
      (rootNote.links as Array<{ targetPath?: string }>).flatMap((link) =>
    ...
    for (const notePath of exampleNotePaths.slice(1)) {
      expect(host.notes.get(notePath)?.links).toEqual(
        expect.arrayContaining([expect.objectContaining({ targetPath: rootNotePath })]),
      );
    }
    const randomValues = [
    ...
    const result = await exportActiveGraph(host, rootNotePath, DEFAULT_SETTINGS, {
