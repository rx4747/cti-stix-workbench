...
generate a Canvas automatically. To work visually, keep `Import Overview.md`
active and run **Generate canvas from current folder**, or create a Canvas
manually and add the imported notes as file nodes.
...
1. Run **Generate canvas from active STIX graph** or **Generate canvas from
   current folder**, or create an Obsidian Canvas manually.
...
images, links, and unlabeled edges remain visual context. Bundle import and
Canvas generation remain separate explicit actions, and arranging the STIX
viewer does not create or update a Canvas. See [Canvas semantics](canvas.md).

## Validate, inspect, and export

For the usual investigation-led workflow, open the typed investigation Note
whose `object_refs` link the objects you intend to share, then use the active
graph commands. With the default settings, direct referenced notes enter that
scope. Increase link depth for a deeper connected graph, or use folder or
Canvas export when it represents the intended sharing boundary more precisely.
