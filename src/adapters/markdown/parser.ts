...
import type { NormalizedStixDraft, ResolvedLink } from "../../core/types";
...
export function parseMarkdownNote(input: unknown): MarkdownParseResult {
  if (!isRecord(input)) {
    return {
      quarantinedKeys: [],
      diagnostics: [inputDiagnostic("A note input must be an object.")],
    };
  }
