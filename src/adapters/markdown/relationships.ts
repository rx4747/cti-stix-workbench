...
import { createDiagnostic, DIAGNOSTIC_CODES } from "../../core/diagnostics";
import type { RelationshipDeclaration, ResolvedLink } from "../../core/types";
import type { MarkdownFence } from "./fences";
import { closesMarkdownFence, readMarkdownFence } from "./fences";
...
function linkIsResolved(target: string, links: readonly ResolvedLink[]): boolean {
  return links.some(
    (link) =>
      link.targetPath !== undefined && normalizeWikiLinkTarget(link.raw) === target,
  );
}
