...
import { createIdentifierService, type IdentifierResult } from "./identifiers";

export const STIX_SCO_UUID_NAMESPACE = "00abedb4-aa42-466c-9c01-fed23315a9b7";
...
const HASH_CONTRIBUTING_TYPES = new Set(["artifact", "file", "x509-certificate"]);
const PREFERRED_HASH_ALGORITHMS = ["MD5", "SHA-1", "SHA-256", "SHA-512"] as const;
...
function diagnostic(
  code:
    | typeof DIAGNOSTIC_CODES.fieldRequired
  ...
  for (let index = 0; index < value.length; index += 1) {
    ...
    if (codeUnit >= 0xd800 && codeUnit <= 0xdbff) {
      ...
      if (!(next >= 0xdc00 && next <= 0xdfff)) {
        throw new TypeError("Canonical JSON cannot contain a lone Unicode surrogate.");
      }
      index += 1;
...
function selectOneHash(value: unknown): unknown {
  ...
  const selected =
    PREFERRED_HASH_ALGORITHMS.find((algorithm) => Object.hasOwn(hashes, algorithm)) ??
    available[0];
...
function uuidToBytes(uuid: string): Uint8Array {
  ...
  for (let index = 0; index < bytes.length; index += 1) {
    ...
    bytes[index] = Number.parseInt(hexadecimal.slice(offset, offset + 2), 16);
  }
...
async function createUuid5(name: string, sha1: Sha1Digest): Promise<string> {
