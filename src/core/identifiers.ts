...
export interface IdentifierService {
  readonly ensureUuid4: (type: string, existingId?: string) => IdentifierResult;
  readonly now: () => string;
}
...
export function validateStixIdentifier(
  ...
  const actualType = separatorIndex === -1 ? undefined : id.slice(0, separatorIndex);
  const uuid = separatorIndex === -1 ? undefined : id.slice(separatorIndex + 2);
